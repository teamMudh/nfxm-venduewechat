            ...
            <div class="info_item" v-for="(acceptance,index) in acceptanceList" :key="acceptance.AC" >
                <div class="info_item_top">
                    <div class="info_item_top_left">
                        <span class="span_name">验收单号</span>
                        <span class="span_value">{{acceptance.AC}}</span>
                    </div>
                    <div class="info_item_top_mid">
                        <span class="span_name">申请数量</span>
                        <span class="span_value">{{acceptance.QTY}}</span>
                    </div>
                    <div class="info_item_top_right">
                        <span class="span_name">申请人</span>
                        <span class="span_value">{{acceptance.CO}}</span>
                    </div>
                </div>
                <div class="info_item_bottom">
                    <div class="info_item_bottom_">
                        <span class="span_name" style=" padding-left: 1%;">验收状态</span>
                        <span class="status" style=" padding-left: 1%;">{{acceptance.AS | formatStatusAcceptance}}</span>
                    </div>

                    <div class="info_item_bottom_m">
                        <span class="span_name">申请时间</span>
                        <span class="span_value">{{ acceptance.CT | formatDateTime }}</span>
                    </div>
                    <div class="info_item_bottom_button">
                        <button @click.prevent="toDetail(acceptance.AC)">查看</button>
                    </div>
                </div>
            </div>
        </div>
